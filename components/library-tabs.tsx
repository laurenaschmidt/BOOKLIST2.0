...
export function LibraryTabs({ entries }: { entries: Record<ReadingStatus, LibraryEntry[]> }) {
  return (
    <Tabs.Root defaultValue="CURRENTLY_READING" className="mt-8">
        ...
        <Tabs.Content key={tab.value} value={tab.value} className="pt-8">
          {entries[tab.value].length === 0 ? (
            <p className="text-ink-muted">
              ...
              <a href="/search" className="font-medium text-accent hover:text-accent-hover">
                search
              </a>{" "}
              to add a book.
            </p>
